...
describe('SpeechInputButton', () => {
  it('dictates into a live draft, toggles state, and never submits', () => {
    ...
    assert.equal(view.root.findAllByType('circle').length, 0)
    ...
    assert.equal(waveform(view).findAllByType('i').length, 16)
    ...
    assert.equal(waveform(view).findAllByType('i').at(-1).props.style.height, '27px')
